...
function safeParse(str: string | null): Record<string, unknown> {
  try { return str ? (JSON.parse(str) as Record<string, unknown>) : {}; } catch { return {}; }
}
...
export default function uploadDocumentsRoutes({ truv, db, apiLogger }: RouteDeps): Router {
  const router = Router();

  router.post('/api/collections', async (req, res) => {
    try {
      const body = (req.body ?? {}) as Record<string, unknown>;
      const { external_user_id, use_test_docs, extra_documents } = body;
      let documents = body.documents as TestDoc[] | undefined;

      // Load test docs if flagged, then append any extra user-uploaded documents
      if (use_test_docs) {
        documents = loadTestDocs();
      }
      const extras = extra_documents as TestDoc[] | undefined;
      if (extras?.length) {
      ...
      apiLogger.logApiCall({ userId, method: 'POST', endpoint: '/v1/documents/collections/', requestBody: { documents: docsWithUser.map(d => ({ filename: d.filename, mime_type: d.mime_type, user_id: userId })) }, responseBody: truvData, statusCode: result.statusCode, durationMs: result.durationMs });
  ...
  router.get('/api/collections/:id', async (req, res) => {
    try {
      ...
      if (collection.truv_collection_id) {
        ...
        const raw = safeParse(collection.raw_response);
        const uploadedFiles = raw.uploaded_files as Array<{ user_id?: string }> | undefined;
        const uid = uploadedFiles?.[0]?.user_id ?? null;
      ...
      const raw = safeParse(updated.raw_response);
