...
export default function ordersRoutes({ truv, db, apiLogger }: RouteDeps): Router {
  const router = Router();

  router.get('/api/orders', (req, res) => {
    try {
      const demoId = req.query.demo_id as string | undefined;
  ...
  router.post('/api/orders', async (req, res) => {
    try {
      ...
      db.createOrder({ orderId, truvOrderId: truvData.id, userId, demoId: (data.demo_id as string | undefined) ?? 'default', bridgeToken: truvData.bridge_token, shareUrl: truvData.share_url, status: truvData.status ?? 'created', rawResponse: truvData });
      db.updateOrder(orderId, { product_type: Array.isArray(data.products) ? (data.products as string[]).join(',') : pt });
