...
interface Earning { name: string; amount: number }
...
function PayStatement({ st }: { st: Statement }) {
    ...
    <div class="border border-border rounded-lg mb-3 overflow-hidden">
      ...
      {open && (
        <div class="px-4 py-3">
          ...
          <Row label="Hours" value={String(st.hours ?? '-')} />
          <Row label="Regular" value={st.regular != null ? $(st.regular) : '-'} />
          <Row label="Overtime" value={st.overtime != null ? $(st.overtime) : '-'} />
          {(st.earnings?.length ?? 0) > 0 && (
            <>
...
interface AnnualSummary { year: number; gross_pay: number; net_pay: number; regular?: number; overtime?: number; other_pay?: number }
interface BankAccount { bank_name?: string; account_type?: string; account_number?: string; deposit_type?: string; deposit_value?: number }
interface W2 { year: number; wages?: number; federal_tax?: number; social_security_tax?: number; medicare_tax?: number; file?: string }
interface Profile { first_name?: string; last_name?: string; full_name?: string; email?: string; date_of_birth?: string; ssn?: string; home_address?: { street?: string; city?: string; state?: string; zip?: string } }
interface Company { name?: string; phone?: string; address?: { street?: string; city?: string; state?: string; zip?: string } }
...
interface ReportLink {
  ...
  provider?: { name?: string; logo_url?: string };
  data_source?: string;
  is_suspicious?: boolean;
}

export function VoieReport({ report }: { report?: { links?: ReportLink[] } }) {
        ...
        <div key={`${li}-${ei}`}>
          ...
          {emp.job_title && (
            <Section title="Employment">
              ...
              {emp.income != null && <Row label="Annual Income" value={$(emp.income)} />}
              {emp.pay_rate != null && <Row label="Pay Rate" value={`${$(emp.pay_rate)} ${freq(emp.pay_frequency)}`} />}
                ...
                <tbody>
                  {annualSummary.map((yr, i) => (
                    <tr key={i} class="border-b border-border-light">
                      ...
                      <td class="px-3 py-2">{yr.regular != null ? $(yr.regular) : '-'}</td>
                      <td class="px-3 py-2">{yr.overtime != null ? $(yr.overtime) : '-'}</td>
                      <td class="px-3 py-2">{yr.other_pay != null ? $(yr.other_pay) : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody>
                  {w2s.map((w, i) => (
                    <tr key={i} class="border-b border-border-light">
                      ...
                      <td class="px-3 py-2">{w.wages != null ? $(w.wages) : '-'}</td>
                      <td class="px-3 py-2">{w.federal_tax != null ? $(w.federal_tax) : '-'}</td>
                      <td class="px-3 py-2">{w.social_security_tax != null ? $(w.social_security_tax) : '-'}</td>
                      <td class="px-3 py-2">{w.medicare_tax != null ? $(w.medicare_tax) : '-'}</td>
