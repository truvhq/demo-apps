...
import type { PanelState, ApiLogRow, BridgeEvent, WebhookEventRow } from './hooks';
...
function GuideTab({ steps, currentStep }: { steps: Step[]; currentStep: number }) {
  return (
    <div>
      {steps.map((step, i) => {
...
function ApiTab({ logs }: { logs: ApiLogRow[] }) {
    ...
    <div>
      {logs.map((log, i) => (
        <LogEntry
          ...
          status={log.status_code != null ? String(log.status_code) : ''}
...
function BridgeEventEntry({ evt }: { evt: BridgeEvent }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div class="mb-2 border border-border-light rounded-lg overflow-hidden">
      <div class={`flex items-center gap-2 px-3 py-2.5 text-xs ${hasData ? 'cursor-pointer hover:bg-gray-50' : ''}`} onClick={() => hasData && setOpen(!open)}>
        ...
        <span class="text-[11px] text-gray-400">{new Date(evt.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
      </div>
      {open && hasData && (
        <div class="border-t border-border-light p-3 bg-gray-50">
...
function BridgeTab({ events }: { events: BridgeEvent[] }) {
  if (!events.length) return <div class="flex items-center justify-center h-32 text-gray-400 text-sm">No Bridge events yet.</div>;
  return <div>{[...events].reverse().map((evt, i) => <BridgeEventEntry key={i} evt={evt} />)}</div>;
}

function WebhookEntry({ wh }: { wh: WebhookEventRow }) {
  const [open, setOpen] = useState(false);
  ...
  const ts = wh.received_at ?? wh.created_at ?? '';
  ...
  return (
    <div class="mb-2 border border-border-light rounded-lg overflow-hidden">
      <div class="flex items-center gap-2 px-3 py-2.5 text-xs cursor-pointer hover:bg-gray-50" onClick={() => setOpen(!open)}>
        ...
        <span class="text-[11px] text-gray-400">{ts ? new Date(ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : ''}</span>
      </div>
      {open && (
        <div class="border-t border-border-light p-3 bg-gray-50">
...
function WebhooksTab({ webhooks, tunnelUrl }: { webhooks: WebhookEventRow[]; tunnelUrl: string | null }) {
  return (
    <div>
      {tunnelUrl && (
...
function tryFormat(s: unknown): string {
  if (s == null) return '';
...
export function Panel({ steps, panel }: { steps: Step[]; panel?: PanelState }) {
  const [activeTab, setActiveTab] = useState('guide');
  const { currentStep = 0, apiLogs = [], bridgeEvents = [], webhooks = [], tunnelUrl = null } = panel ?? {};

  const tabs: Tab[] = [
    { id: 'guide', label: 'Guide', count: 0 },
