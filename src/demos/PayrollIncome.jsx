...
const DIAGRAM = `sequenceDiagram
  ...
  Note right of Truv: data_sources: [payroll]
...
const STEPS = [
  {
    ...
    guide: '<p>The form collects applicant PII and employer. An order is created with:</p>'
      + '<pre>POST /v1/orders/</pre>'
      + '<p>The order uses <code>data_sources: ["payroll"]</code> to route through payroll verification.</p>'
  ...
  { title: 'Retrieve results', guide: '<p>Fetch the VOIE report:</p><pre>POST /v1/users/{user_id}/reports/</pre><p><a href="https://docs.truv.com/reference/users_reports" target="_blank">Reports API →</a></p>' },
];

export function PayrollIncomeDemo({ screen, param }) {
  ...
  async function handleSubmit(formData) {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/orders`, {
        ...
        body: JSON.stringify({ ...formData, demo_id: 'payroll-income', data_sources: ['payroll'] }),
      });
      const data = await resp.json();
      if (!resp.ok) { alert('Error: ' + (data.error || 'Unknown')); setSubmitting(false); return; }
      navigate(`consumer-credit/payroll-income/bridge/${data.order_id}`);
  ...
  return (
    <Layout title="Truv Quickstart" badge="Payroll Income" steps={STEPS} panel={panel} flush={isBridge} hidePanel={isIntro}>
      {screen === 'bridge' && (
        <BridgeScreen orderId={orderId} demoPath="consumer-credit/payroll-income" addBridgeEvent={addBridgeEvent} startPolling={startPolling} />
      )}
      {screen === 'waiting' && (
        <OrderWaitingScreen orderId={param} demoPath="consumer-credit/payroll-income" webhooks={panel.webhooks} startPolling={startPolling} />
      )}
      {screen === 'results' && (
        <OrderResultsScreen orderId={param} onBack={() => { reset(); setIntroStep(1); navigate('consumer-credit/payroll-income'); }} backLabel="New Verification" />
      )}
      {!screen && (
        introStep === 1 ? (
          <div class="intro-slide">
            <div class="relative z-10 w-full max-w-2xl mx-auto px-4">
              <div class="animate-slideUp">
                ...
                <h2 class="text-[36px] font-semibold tracking-[-0.03em] leading-[1.1] text-[#1d1d1f] mb-4">Verify income from payroll data</h2>
                <p class="text-[17px] text-[#86868b] leading-[1.5] max-w-[440px] mx-auto mb-7">
                  Connect to a payroll provider and verify income, employment, and pay history.
                </p>
              </div>
          ...
          <IntroSlide
            label="Payroll Income Verification"
            title="Architecture"
            subtitle="How the payroll income verification flow works end-to-end."
            diagram={DIAGRAM}
          >
          ...
          <div class="max-w-lg mx-auto">
            <ApplicationForm onSubmit={handleSubmit} submitting={submitting} productType="income" />
          </div>
        )
      )}
    </Layout>
