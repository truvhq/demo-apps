...
interface OrderResult { order_id: string; [key: string]: unknown }

export function FollowUpDemo({ screen, param }: DemoProps) {
  const [applicationId, setApplicationId] = useState(() => `qs-${Date.now()}`);
  const [taskOrders, setTaskOrders] = useState<Record<string, OrderResult> | null>(null);
  ...
  async function handleInitialize() {
    ...
    for (const task of TASKS) {
      try {
        const body: Record<string, unknown> = { products: task.products, demo_id: 'follow-up', external_user_id: applicationId.trim() };
      ...
      {screen === 'bridge' && (
        <BridgeScreen
          ...
          onCompleted={() => {
            ...
            if (taskId) setTaskStatus(prev => ({ ...prev, [taskId]: 'completed' }));
          }}
        />
      )}
      {screen === 'waiting' && (
    ...
    <div class="intro-slide">
      <div class="relative z-10 w-full max-w-2xl mx-auto px-4">
        ...
        <div class="animate-slideUp delay-2 w-full max-w-xs mx-auto">
          ...
          <input
            value={applicationId}
            onInput={e => onApplicationIdChange((e.target as HTMLInputElement).value)}
...
function TaskList({ tasks, taskOrders, taskStatus, onStart }: {
  tasks: Task[];
  taskOrders: Record<string, OrderResult>;
