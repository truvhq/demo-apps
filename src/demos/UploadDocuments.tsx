...
import { Layout, OrderResults, WebhookFeed, usePanel, API_BASE, IntroSlide } from '../components/index';
import { parsePayload } from '../components/WebhookFeed';
...
interface CollectionStatusResponse {
  raw_response?: {
    uploaded_files?: Array<{ status: string }>;
  };
}
...
export function UploadDocumentsDemo(_: DemoProps) {
  ...
  async function processDocuments() {
    ...
    try {
      ...
      const body: {
        ...
        use_test_docs: boolean;
        extra_documents?: Array<{ filename: string; mime_type: string; content: string }>;
      } = { external_user_id: userId.trim() || undefined, use_test_docs: true };
      if (files.length > 0) {
      ...
      const createData = await createResp.json() as CollectionCreateResponse;
      ...
      if (!cid) { alert('Error: missing collection_id'); setProcessing(false); return; }
      setCollectionId(cid);
      ...
      const waitForFiles = async () => {
        try {
          ...
          const data = await resp.json() as CollectionStatusResponse;
          const uploadedFiles = data.raw_response?.uploaded_files ?? [];
          const allOk = uploadedFiles.length > 0 && uploadedFiles.every(f => f.status === 'successful');
  ...
  useEffect(() => {
    ...
    const doneWebhook = panel.webhooks.find(w => {
      ...
      return (p['event_type'] === 'task-status-updated' && p['status'] === 'done')
        || (w.event_type === 'task-status-updated' && (w as unknown as Record<string, unknown>)['status'] === 'done');
    });
    if (doneWebhook && !linkIdRef.current) {
      const p = parsePayload(doneWebhook.payload);
      const linkId = (p['link_id'] ?? (doneWebhook as unknown as Record<string, unknown>)['link_id']) as string | undefined;
      if (!linkId) return;
...
function UploadScreen({ files, onAddFiles, onRemoveFile, userId, onUserIdChange, onBack, onContinue, processing }: UploadScreenProps) {
      ...
      <div class="w-full max-w-lg mx-auto px-4">
        ...
        <div class="animate-slideUp delay-1 text-left">
          ...
          <div
            ...
            onDrop={e => { e.preventDefault(); setDragOver(false); onAddFiles(e.dataTransfer?.files ?? null); }}
          ...
          >
            ...
            <input ref={inputRef} type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.tiff,.tif" class="hidden" onChange={e => { onAddFiles((e.target as HTMLInputElement).files); (e.target as HTMLInputElement).value = ''; }} />
          </div>
          ...
          <input
            value={userId}
            onInput={e => onUserIdChange((e.target as HTMLInputElement).value)}
