...
import { Layout, OrderResults, WebhookFeed, usePanel, API_BASE, parsePayload, IntroSlide } from '../components/index';
...
export function EmployeePortalDemo({ screen: _screen, param: _param }: DemoProps) {
  const [introStep, setIntroStep] = useState(1);
  const [introSeen, setIntroSeen] = useState(false);
  const [testApplicant, setTestApplicant] = useState<Applicant | null>(null);
  ...
  async function handleRequest() {
    ...
    try {
      ...
      const data = await resp.json() as OrderResult;
      if (resp.ok) {
        setOrder(data);
        setCurrentStep(1);
        ...
        <div class="max-w-4xl mx-auto px-8 py-10">
          ...
          {reportData && (
            <div class="border border-[#d2d2d7] rounded-xl p-6 bg-white">
              ...
              <OrderResults data={reportData as Parameters<typeof OrderResults>[0]['data']} />
            </div>
          )}
...
function AddApplicantForm({ onSubmit }: { onSubmit: (applicant: Applicant) => void }) {
        ...
        <form onSubmit={handleSubmit} class="animate-slideUp delay-1 text-left">
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input value={firstName} onInput={e => setFirstName((e.target as HTMLInputElement).value)} placeholder="John" class="w-full px-3.5 py-2.5 border border-[#d2d2d7] rounded-lg text-sm focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label class="text-[13px] font-medium text-[#1d1d1f] mb-1.5 block">Last name</label>
              <input value={lastName} onInput={e => setLastName((e.target as HTMLInputElement).value)} placeholder="Doe" class="w-full px-3.5 py-2.5 border border-[#d2d2d7] rounded-lg text-sm focus:border-primary focus:outline-none" />
            </div>
          </div>
          <div class="mb-3">
            <label class="text-[13px] font-medium text-[#1d1d1f] mb-1.5 block">Email</label>
            <input type="email" value={email} onInput={e => setEmail((e.target as HTMLInputElement).value)} placeholder="john@example.com" class="w-full px-3.5 py-2.5 border border-[#d2d2d7] rounded-lg text-sm focus:border-primary focus:outline-none" />
          ...
          <div class="mb-3">
            ...
            <input type="tel" value={phone} onInput={e => setPhone((e.target as HTMLInputElement).value)} placeholder="+14155551234" class="w-full px-3.5 py-2.5 border border-[#d2d2d7] rounded-lg text-sm focus:border-primary focus:outline-none" />
          ...
          <div class="mb-5">
            ...
            <select value={product} onChange={e => setProduct((e.target as HTMLSelectElement).value)} class="w-full px-3.5 py-2.5 border border-[#d2d2d7] rounded-lg text-sm bg-white focus:border-primary focus:outline-none">
