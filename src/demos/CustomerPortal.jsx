...
export function CustomerPortalDemo({ screen, param }) {
  ...
  return (
    <Layout title="Truv Quickstart" badge="Customer Portal" steps={STEPS} panel={panel} flush={isBridge} hidePanel={isIntro}>
      {screen === 'bridge' && (
      ...
      {screen === 'results' && (
        <OrderResultsScreen orderId={param} onBack={() => { reset(); setProductType(null); navigate('public-sector/customer-portal'); }} backLabel="Customer Portal" />
      )}
      {!screen && (
        productType ? (
...
function CPIntroScreen({ onStart }) {
  ...
  if (step === 2 && selected) {
    return (
      <IntroSlide
        label={`Customer Portal → ${CP_PRODUCTS.find(p => p.id === selected)?.name}`}
        title="Architecture"
        subtitle={`How the ${selected} verification flow works end-to-end.`}
        diagram={CP_DIAGRAMS[selected]}
      >
    ...
    <div class="intro-slide">
      <div class="relative z-10 w-full max-w-2xl mx-auto px-4">
        <div class="animate-slideUp">
          <div class="text-[12px] font-medium uppercase tracking-[0.08em] text-primary mb-4">Customer Portal</div>
          ...
          <p class="text-[17px] text-[#86868b] leading-[1.5] max-w-[440px] mx-auto mb-7">
            The customer fills in their details, selects their employer or bank, and completes verification through Bridge.
          </p>
        </div>
