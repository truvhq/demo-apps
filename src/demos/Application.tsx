...
import { Layout, usePanel, API_BASE, IntroSlide } from '../components/index';
import { CompanySearch } from '../components/CompanySearch';
import { BridgeScreen, OrderWaitingScreen, OrderResultsScreen } from '../components/screens/index';
...
const STEP_MAP: Record<string, number> = { '': 0, 'bridge': 1, 'waiting': 2, 'results': 3 };

export function ApplicationDemo({ screen, param }: DemoProps) {
  const [productType, setProductType] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const { panel, setCurrentStep, startPolling, addBridgeEvent, reset } = usePanel();

  useEffect(() => {
    setCurrentStep(STEP_MAP[screen] ?? 0);
  }, [screen]);

  async function handleSubmit(formData: Record<string, string | undefined>) {
    setSubmitting(true);
    try {
      ...
      const data = await resp.json() as { order_id?: string; error?: string };
...
function IntroScreen({ onStart }: { onStart: (productId: string) => void }) {
...
interface SelectedEmployer { name: string; id: string | null }
...
function ApplicationForm({ onSubmit, submitting, productType }: {
  ...
  const [employer, setEmployer] = useState<SelectedEmployer>({ name: '', id: null });
  ...
  const handleSubmit = (e: Event) => {
    ...
    onSubmit({
      ...
      ssn: fd.get('ssn') as string | undefined,
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div class="mb-4">
        ...
        <CompanySearch value={employer.name} onChange={e => setEmployer({ name: e.name, id: e.id })} productType={productType} placeholder={isAssets ? 'Search for bank or institution...' : 'Search for employer...'} />
      ...
      <label class="flex items-center gap-2.5 my-6 cursor-pointer text-sm text-gray-500">
        <input type="checkbox" checked={agree} onChange={e => setAgree((e.target as HTMLInputElement).checked)} class="w-4.5 h-4.5 accent-primary" />
