...
/** Same shared truv_integration config LOS's Documents tab uses for its
 * "Generate AIM Check Report" button — POS doesn't have that button itself
 * (AIM Check documents are generated from LOS), but this keeps the setting
 * editable from either project, same pattern as credentials. Truv
  ...
  return (
    <Card title="AIM Check Configuration" actions={<Button onClick={handleSave} loading={saving}>Save</Button>}>
      <p style={{ color: 'var(--truv-grey-50)', fontSize: 13, marginTop: -8 }}>
        Controls what LOS's "Generate AIM Check Report" (Documents tab) actually does — shared config, editable
        from either project.
      </p>
      <Select
        label="Flow"
