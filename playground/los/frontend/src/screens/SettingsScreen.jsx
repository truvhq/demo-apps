...
export function SettingsScreen() {
  ...
  async function handleActivate(id) {
    setBusyId(id);
    try { await api.activateCredentialSet(id); refresh(); } finally { setBusyId(null); }
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 800 }}>
        <Card title="Truv Credentials">
          <p style={{ color: 'var(--truv-grey-50)', fontSize: 13, marginTop: -8 }}>
            LOS has its own credential store, independent of POS's — enter the same sandbox/production keys here so
            LOS can call Truv directly for refreshes.
          </p>
          {credentialSets === null && <p>Loading…</p>}
          {credentialSets && credentialSets.length === 0 && <p style={{ color: 'var(--truv-grey-50)' }}>No credential sets yet.</p>}
          {credentialSets && credentialSets.map((cred) => (
            editingId === cred.id ? (
              ...
              <div key={cred.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid var(--truv-grey-30)', borderRadius: 12, padding: 16 }}>
                <div>
